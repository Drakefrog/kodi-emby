...
|---|---|---|---|
| Emby for Kodi Next-Gen | MediaBrowser/plugin.video.emby, `next-gen-dev-python3` | `4051767ec4fde5075b07cac7cdfab7e1061d9d89` | `vendor/emby-next-gen`; upstream `LICENSE.txt` retained |
...
The installed custom HEADs are evidence only, not upstream commits: Next-Gen `5d2aa07`, EmbyCon `d445bca`, Fuse `c89780b`. They are never used for update detection. An earlier local-only Next-Gen snapshot (`ae994cd`) was removed from provenance because the upstream no longer serves it; the fetchable branch commit above is the reproducible base.
